...
async function saveCanvas(
  ...
  const content = format === "json" ? JSON.stringify(canvas, null, 2) : yaml.stringify(canvas);
...
async function main() {
  ...
  try {
    ...
    const hasColors = extendedDemo.nodes.some((n) => n.color) ||
      extendedDemo.edges.some((e) => e.color);
